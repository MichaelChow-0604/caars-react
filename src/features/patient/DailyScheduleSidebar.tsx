...
import { IconChevronDown, IconChevronLeft, IconChevronRight, IconTask } from '@/lib/icon';
...
export default function DailyScheduleSidebar({
  ...
  const appointmentsForDate = getAppointmentsForDate(FAKE_APPOINTMENTS, selectedDate);
  ...
  const slotToAppointment = new Map<string, (typeof appointmentsForDoctor)[0]>();
      ...
      {collapsed ? (
        ...
        <div className="flex-1 min-h-0 overflow-y-auto py-2 px-2 flex flex-col gap-2">
          {slots.map((slot) => {
            ...
            const isCurrentPatient = apt && patientId && apt.extendedProps.patientId === patientId;
              ...
              <div key={timeKey} className="flex flex-col gap-2 shrink-0">
                ...
                {apt ? (
                  <AppointmentPreview
                    ...
                    onClick={() => handleAppointmentClick(apt.extendedProps.patientId)}
                  />
                ) : (
